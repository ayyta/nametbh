...
export async function GET(req) {
    ...
    if (!pfpId && !bannerId) {
      return new Response(
        JSON.stringify({ error: "At least one of pfpId or bannerId is required" }),
        { status: 400 }
      );
    }
...
export async function POST(req) {
    ...
    try {
      ...
      const fieldToUpdate = apiCall === "pfp" ? { pfp: mediaData.media_id } : { profile_background: mediaData.media_id };
      ...
      if (userUpdateError) {
        throw new Error(`Error updating user profile picture: ${userUpdateError.message}`);
      }
