...
export default function Register() {
  ...
  let emailUsernameErrorMsg = logInError
    ? ` - ${error ? error : 'Invalid Input'}`
    : '*';
  let passwordErrorMsg = logInError
  ...
  useEffect(() => {
    ...
    emailUsernameErrorMsg = logInError
      ? ` - ${error ? error : 'Invalid Input'}`
      : '*';
    passwordErrorMsg = logInError
      ? ' - Password must be atleast 6 characters'
      : '*';
  }, [logInError]);
  return (
    <>
      <div className="w-screen h-screen md:bg-login bg-loginPopup flex justify-center md:items-center ">
        ...
        <form
          ...
          className={`md:w-138 md:h-auto md:rounded-3xl md:px-16 md:py-7 py-16 w-full min-w-min	px-10 bg-loginPopup flex flex-col ${inter.className}`}
        >
          <p className="md:text-center md:text-2xl mb-4 text-2xl text-center font-bold ">
            Create an account
          </p>
          ...
          <InputComponent
            ...
            headerErrorStyle={headerErrorStyle}
            errorMsg={errorMsg}
          />
          <InputComponent
            name="email"
            ...
            headerStyle={headerStyle}
            headerErrorStyle={headerErrorStyle}
            errorMsg={emailUsernameErrorMsg}
          />
          <InputComponent
            name="username"
            value={username}
          ...
          {/* Continue Button */}
          <button
            className="mt-5 w-full md:h-12 rounded-md h-12 bg-loginButton text-projectWhite font-bold"
            type="submit"
            disabled={isLoading}
          >
          ...
          <div className="w-full">
            <Link
              href={loginPath}
              className="md:text-sm text-xs mt-1 text-projectBlue hover:text-projectHoverBlue cursor-pointer"
            >
              Have an account?
            </Link>
          </div>
  ...
  return (
    <>
      <label className={`flex flex-col ${style}`}>
        <p className={`md:text-sm uppercase text-xs font-bold ${headerStyle}`}>
          {name}
          <span className={`md:text-xs text-red-600 ${headerErrorStyle}`}>
            {errorMsg}
          </span>
        </p>
        <input
          className="md:w-full md:h-12 rounded-md mt-1 h-10 text-projectWhite bg-login pl-3"
          required
          value={value}
  ...
  return (
    <label className={`flex flex-col ${style}`}>
      ...
      <div className="flex flex-row md:w-full md:h-12  mt-1 rounded-md h-10 text-projectWhite bg-login">
        <input
          className="w-full h-full rounded-md pl-3 bg-transparent"
          required
          value={value}
        ...
        />
        <button
          className="px-2 text-projectWhite text-sm"
          type="button"
    ...
    <>
      <label className="flex flex-col pt-5">
        ...
        <div className="flex gap-2 relative">
          ...
          <DropUp
            value={value.month}
            setValue={(option) => handleDateChange('month', option)}
            placeHolder="Month"
            options={months}
          />
          <DropUp
            value={value.day}
            setValue={(option) => handleDateChange('day', option)}
            placeHolder="Day"
            options={days}
          />
          <DropUp
            value={value.year}
            setValue={(option) => handleDateChange('year', option)}
            placeHolder="Year"
            options={years}
          />
        </div>
...
function DropUp({ value, setValue, placeHolder, options }) {
  ...
  const dateStyle =
    'w-1/3 md:h-12 rounded-md mt-1 h-10 text-projectWhite bg-login px-2 relative flex items-center cursor-pointer';
  ...
  return (
    ...
    >
      ...
      <div className={`${value ? '' : 'text-projectGrey'} flex-1 flex`}>
        {value || placeHolder}
        <span
          className={`arrow ${isOpen ? 'open' : ''} flex-1 w-full md:flex justify-end hidden`}
        >
          &#9650;
        </span>
      </div>
