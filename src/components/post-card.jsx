...
export default function Component({
  ...
  const handleLike = (isActive, count) => {
    console.log(
      `Like is now ${isActive ? 'active' : 'inactive'} with count: ${count}`,
    );
  };

  const handleDislike = (isActive, count) => {
    console.log(
      `Dislikes is now ${isActive ? 'active' : 'inactive'} with count: ${count}`,
    );
  };
...
const PostCardInteractionButton = forwardRef(function PostCardButton(
  ...
  return (
    ...
    >
      ...
      />
      <span className={`text-sm  ${isActive ? `text-${color}` : 'text-white'}`}>
        {count}
      </span>
    </Button>
  );
